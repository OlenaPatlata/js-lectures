...
class ApiService {
  ...
  getUsersList() {
    ...
    return fetch(`${ApiService.BASE_URL}/users?${params}`)
      .then(res => {
        if (res.status === 404) {
  ...
  getUserByName() {
    return fetch(`${ApiService.BASE_URL}/users/${this.#userName}`).then(res => {
      if (res.status === 404) {
        return Promise.reject(new Error(`User ${this.#userName} not found`));
      }
      return res.json();
    });
  }
